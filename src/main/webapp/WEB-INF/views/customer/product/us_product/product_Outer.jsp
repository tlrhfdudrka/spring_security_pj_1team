...
<head>
	...
	<link rel="stylesheet" href="${path}/resources/css/customer/product_Outer.css" />
								...
								<ul>
									<li id="uspd_outer_page">
										...
										<c:if test="${paging2.startPage > 10}">
											<a href="${path}/product_Outer.uspd?category_num=1&pageNum=${paging2.prev}">◀</a>
										</c:if>
										
										<!-- 페이지 번호 처리 -->
										<c:forEach var="num" begin="${paging2.startPage}" end="${paging2.endPage}">
											<a href="${path}/product_Outer.uspd?category_num=1&pageNum=${num}">${num}</a>
										</c:forEach>
										
										<!-- 다음 버튼 활성화 -->
										<c:if test="${paging2.endPage < paging2.pageCount}">
											<a href="${path}/product_Outer.uspd?category_num=1&pageNum=${paging2.next}">▶</a>
										</c:if>
									</li>
								</ul>
							</form>
